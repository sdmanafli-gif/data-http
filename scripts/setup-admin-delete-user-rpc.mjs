/**
 * Admin delete-user RPC (avoids Edge Function).
 * Reuses / stores service role secrets in private.app_secrets.
 *
...
const sqlPath = path.join(root, 'setup', 'tables', '31_admin_delete_user_rpc.sql')
  ...
  const check = await client.query(
    `select proname from pg_proc
     where pronamespace = 'public'::regnamespace
       and proname = 'admin_delete_user'`
  )
  console.log('OK RPC:', check.rows.map((r) => r.proname).join(', ') || 'missing')
