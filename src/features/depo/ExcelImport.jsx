...
import { DEPO_TABLE, toDepoPayload } from './constants'
import {
  parseExcelFile,
  suggestMapping,
    ...
    try {
      for (let i = 0; i < rows.length; i++) {
        try {
          ...
          const { error: insErr } = await supabase.from(DEPO_TABLE).insert(payload)
  ...
  return (
    <div className="card">
      <div className="page-header" style={{ marginBottom: 16 }}>
        <h2 className="card__title" style={{ margin: 0 }}>Depo — Excel idxal</h2>
        ...
        <>
          ...
          <div className="table-wrap" style={{ marginBottom: 16 }}>
            <table className="data-table">
              <thead>
                <tr><th>Excel</th><th>Nümunə</th><th>Depo sahəsi</th></tr>
              </thead>
              <tbody>
                {headers.map((h) => (
                  <tr key={h}>
      ...
      {step === 3 && result && (
        <div style={{ marginTop: 16 }}>
          ...
          <button type="button" className="btn btn--primary" onClick={() => navigate('/depo')}>Depoya qayıt</button>
        </div>
      )}
    </div>
  )
